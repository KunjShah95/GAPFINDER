...
export default function PricingPage() {
  ...
  return (
    <div className="min-h-screen bg-[rgb(var(--glass-bg))] py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[rgb(249 115 22 / 0.1)] border border-[rgb(249 115 22 / 0.2)] text-sm text-[rgb(251 146 60)] mb-6">
            ...
            return (
              <motion.div
                ...
                className={`relative card p-6 flex flex-col gap-4 ${
                  plan.badge ? "border-[rgb(249 115 22)] shadow-xl shadow-[rgb(249 115 22 / 0.1)]" : ""
                } ${isCurrent ? "ring-2 ring-emerald-500/60" : ""}`}
              >
                {plan.badge && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-[rgb(249 115 22)] text-white text-xs font-semibold rounded-full whitespace-nowrap">
                    {plan.badge}
                  </div>
                )}
                {isCurrent && (
                ...
                <div>
                  ...
                  <p className="text-sm text-[rgb(var(--text-secondary))] mt-1">{plan.desc}</p>
                </div>

                <div className="flex items-baseline gap-1">
                  <span className="text-3xl font-bold">{plan.price}</span>
                  {plan.price !== "Custom" && (
                    <span className="text-sm text-[rgb(var(--text-secondary))]">/mo</span>
                  )}
                </div>
                ...
                <button
                  ...
                  className={`mt-auto w-full py-2.5 px-4 rounded-xl font-medium text-sm flex items-center justify-center gap-2 transition-all ${
                    ctaDisabled(plan)
                      ...
                      ? "bg-[rgb(249 115 22)] text-white hover:opacity-90"
                      : plan.ctaVariant === "secondary"
                      ? "bg-[rgb(255 255 255 / 0.06)] border border-[rgb(255 255 255 / 0.08)] hover:bg-[rgb(255 255 255 / 0.08)]"
                      : "border border-[rgb(255 255 255 / 0.08)] hover:bg-[rgb(255 255 255 / 0.06)]"
                  }`}
                >
                  {ctaLabel(plan)}
        ...
        >
          ...
          <div className="overflow-x-auto rounded-2xl border border-[rgb(255 255 255 / 0.08)]">
            <table className="w-full min-w-[700px]">
              <thead>
                <tr className="border-b border-[rgb(255 255 255 / 0.08)]">
                  <th className="text-left px-6 py-4 text-sm font-semibold text-[rgb(var(--text-secondary))] w-2/5">
                    Feature
                  </th>
                  {PLANS.map(plan => (
              ...
              <tbody>
                {PLAN_FEATURES.map((feature, i) => (
                  <tr
                    key={feature.label}
                    className={`border-b border-[rgb(255 255 255 / 0.08)]/50 hover:bg-[rgb(255 255 255 / 0.06)]/30 transition-colors ${
                      i % 2 === 0 ? "" : "bg-[rgb(255 255 255 / 0.06)]/10"
                    }`}
                  >
                    <td className="px-6 py-3.5 text-sm text-[rgb(var(--text-primary))]">{feature.label}</td>
                    {tiers.map(t => (
        ...
        >
          ...
          <p className="text-[rgb(var(--text-secondary))] mb-8 max-w-xl mx-auto">
